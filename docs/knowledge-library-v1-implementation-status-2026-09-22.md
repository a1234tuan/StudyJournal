...
导图改为专题中心的均衡双向布局，节点主体支持鼠标拖拽与触屏长按、同级排序和落点反馈；单击直接打开轻量日志详情，大纲展示节点下的日志引用。全量非 live 单测 229 文件／1802 测试通过，知识库 E2E 37 通过／1 桌面触屏项跳过，类型与生产构建通过。截图亲自复核；未安装或部署。详见 [导图精细化验收记录](knowledge-library-map-refinement-2026-09-23.md)。
