...
当前交付是可用的第一阶段，不等同于完整 Excalidraw 编辑器。元素选择后移动/缩放、框选与分组、复制、层级调整、跨块笔迹裁分、孤立锚点修复、触控笔专用模式和完整快捷键仲裁仍属于后续完善范围。语音阶段 0–6 及 2026-09-09 编辑器布局收口后，当前自动化基线为 Vitest `126` 个文件 / `840` 项测试、Desktop/Android-narrow Playwright `42/42`、Firebase Emulator `4/4`；Android 真机手写、中文 IME 和安全区仍保留人工验收门槛。
