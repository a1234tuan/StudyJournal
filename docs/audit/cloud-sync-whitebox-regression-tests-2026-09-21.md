...
> 本清单保留测试设计时的假设与状态，部分假设已被独立验证推翻，不能直接当作当前缺陷清单。当前修复、断言修正及验收结果见 [白盒复核修复与验收](cloud-sync-whitebox-remediation-2026-09-21.md)，尤其注意 W-07、W-10、W-25 的勘误。
...
## 执行结果（2026-09-21 实跑，详见执行报告）

本清单已于当日全部实现并执行：**47 项用例 / 42 通过 / 5 不通过**。逐项结果、发现详情与三条被证伪的原假设见 `docs/audit/cloud-sync-whitebox-execution-report-2026-09-21.md`。

上表"现状"列是**执行前的预期**，实跑后的差异只有四处，特此标注以免误用：

- **W-02**：通过（"一旦发生就会整包失败"这一断言成立；可达性仍为 [?]）。
- **W-05**：通过，且**原假设被证伪**——`persistLedgers` 写的是 `cloudRevision: entity.revision`（远端文档自身修订），不会因 `unknown` 操作核对而回退。
- **W-10**：通过（两种可注入情形）。第三种"超时但元数据未开始"**不可达**：`publish` 内唯一会超时的位置是元数据批量写（此时 `metadataStarted` 已为 true），锁获取超时时 `lock` 为 `undefined`。
- **W-18**：口径已固定为"拒绝"（存储层严格比较含 `updatedAt` 的全实体），但**页面侧是否总能给出可继续的出口仍需要真机确认**——本轮只钉住了存储层行为。
- 另发现 `persistLedgers` 的第 4 个参数 `revision` 已成为**未使用参数**。

**不通过的五项**（F-1/W-15、F-2/W-16、F-3/W-17b、F-4/W-25、F-5/W-01）均是实现侧问题，用例保留为红色，待修复后转绿。
