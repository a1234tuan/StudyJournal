...
test("adds a log once, preserves remarks through remove and restore, and returns from the log reader", async ({ page }) => {
  ...
  await active(page).getByRole("button", { name: /遍历方法 文字节点/ }).click();
  await active(page).getByRole("button", { name: "关联日志", exact: true }).click();
  await page.getByRole("dialog").getByRole("checkbox").check();
  await page.getByRole("dialog").getByRole("button", { name: "加入所选 1 条日志" }).click();
  ...
  page.once("dialog", dialog => dialog.accept());
  await page.getByRole("dialog").getByRole("button", { name: "加入所选 1 条日志" }).click();
  await active(page).getByText("引用备注", { exact: true }).click();
...
test("retains input when another writer updates the note and never silently approves new candidates", async ({ page }) => {
  ...
  await expect(page.getByRole("dialog").getByRole("status")).toContainText("内容或候选已变化");
