...
describe("planning decontamination (dev plan section 1.5)", () => {
  it("no longer exposes any historical effect summary to the planner", () => {
    const prompt = buildSessionPlanningPrompt({ blocks: [], allowedSupportingDecisionBlockIds: [] });
    // The old prompt told the planner the effect numbers "came from objective grading",
    // which is false: every one of them was AI graded. Neither the field nor the
    // claim may come back.
    expect(prompt).not.toContain("interventionEffects");
    expect(prompt).not.toContain("客观判题");
    expect(prompt).toContain("不得使用跨题目的效果百分比");
  });
});
