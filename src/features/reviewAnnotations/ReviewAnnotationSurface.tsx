...
import { ArrowUpRight, Circle, Diamond, Eraser, Hand, Highlighter, ListFilter, MousePointer2, Pencil, Redo2, RectangleHorizontal, Slash, TextCursorInput, Trash2, Type, Undo2 } from "lucide-react";
...
const formTools: ReviewAnnotationTool[] = ["text", "input", "select"];
type ReviewAnnotationSurfaceTool = ReviewAnnotationTool | "browse" | "selection";
type AnnotationBounds = { left: number; top: number; right: number; bottom: number };
const toolLabels: Record<ReviewAnnotationSurfaceTool, string> = {
  browse: "浏览",
  selection: "选择",
  pen: "画笔",
  highlighter: "荧光笔",
...
export const ReviewAnnotationSurface = ({ recordId, occurrenceKey, contentRevision, children, open: controlledOpen, onOpenChange, viewportOverlayHost }: Props) => {
  ...
  const [tool, setTool] = useState<ReviewAnnotationSurfaceTool>("browse");
  ...
  const interactionRef = useRef<{ mode: "move" | "box" | "resize"; startX: number; startY: number; base: ReviewAnnotationElement[]; ids: string[]; bounds?: AnnotationBounds; handle?: string; last?: ReviewAnnotationElement[] }>();
  ...
  const anchorFor = (clientY: number) => {
    const nodes = blocks();
    const containingIndex = nodes.findIndex((node) => { const rect = node.getBoundingClientRect(); return clientY >= rect.top && clientY <= rect.bottom; });
    const index = containingIndex >= 0 ? containingIndex : nodes.reduce((closest, node, nodeIndex) => {
      const rect = node.getBoundingClientRect();
      const distance = Math.abs(clientY - (rect.top + rect.height / 2));
      const closestRect = nodes[closest]?.getBoundingClientRect();
      const closestDistance = closestRect ? Math.abs(clientY - (closestRect.top + closestRect.height / 2)) : Number.POSITIVE_INFINITY;
      return distance < closestDistance ? nodeIndex : closest;
    }, 0);
  ...
  const screenBounds = (element: ReviewAnnotationElement) => {
    ...
    return {
      left,
      top,
      right: formTools.includes(element.kind) ? Math.max(...xs, left + 120) : Math.max(...xs),
      bottom: formTools.includes(element.kind) ? Math.max(...ys, top + 38) : Math.max(...ys),
    };
  };
  const rootPoints = (element: ReviewAnnotationElement) => {
    const root = rootRef.current?.getBoundingClientRect();
    const rect = nodeFor(element)?.getBoundingClientRect();
    if (!root || !rect) return [];
    const points = element.points.map((point) => ({
      x: rect.left - root.left + point.x * rect.width,
      y: rect.top - root.top + point.y * rect.height,
    }));
    if (formTools.includes(element.kind) && points.length >= 2) {
      const left = Math.min(...points.map((point) => point.x));
      const top = Math.min(...points.map((point) => point.y));
      const right = Math.max(...points.map((point) => point.x), left + 120);
      const bottom = Math.max(...points.map((point) => point.y), top + 38);
      return [{ x: left, y: top }, { x: right, y: bottom }];
    }
    return points;
  };
  const elementFromRootPoints = (element: ReviewAnnotationElement, points: ReviewAnnotationPoint[], targetNode = nodeFor(element), targetIndex = element.anchorIndex) => {
    const root = rootRef.current?.getBoundingClientRect();
    const rect = targetNode?.getBoundingClientRect();
    if (!root || !rect) return element;
    return {
      ...element,
      anchorIndex: targetIndex,
      anchorFingerprint: fingerprint(targetNode),
      points: points.map((point) => ({
        x: (root.left + point.x - rect.left) / Math.max(1, rect.width),
        y: (root.top + point.y - rect.top) / Math.max(1, rect.height),
      })),
      updatedAt: new Date().toISOString(),
    };
  };
  const reanchorElement = (element: ReviewAnnotationElement) => {
    const points = rootPoints(element);
    if (!points.length) return element;
    const centerY = points.reduce((sum, point) => sum + point.y, 0) / points.length;
    const root = rootRef.current?.getBoundingClientRect();
    if (!root) return element;
    const { index, node } = anchorFor(root.top + centerY);
    return elementFromRootPoints(element, points, node, index);
  };
  ...
  const pointerDown = (event: ReactPointerEvent) => {
    ...
    if (tool === "selection") {
      ...
      if (hit) {
        ...
        if (!event.shiftKey && !event.ctrlKey) interactionRef.current = { mode: "move", startX: x, startY: y, base: draft.elements, ids, bounds: selectedBounds(ids) };
      } else {
    ...
    if (tool === "text" || tool === "input" || tool === "select") {
  ...
  const pointerMove = (event: ReactPointerEvent) => {
    if (tool === "selection" && interactionRef.current) {
      const root = rootRef.current?.getBoundingClientRect(); if (!root) return;
      ...
      const requestedDx = x - interaction.startX; const requestedDy = y - interaction.startY; const idSet = new Set(interaction.ids);
      const bounds = interaction.bounds;
      const dx = bounds ? Math.max(-bounds.left, Math.min(requestedDx, root.width - bounds.right)) : requestedDx;
      const dy = bounds ? Math.max(-bounds.top, Math.min(requestedDy, root.height - bounds.bottom)) : requestedDy;
      const transformed = interaction.base.map((element) => {
        if (!idSet.has(element.id)) return element;
        const points = rootPoints(element);
        if (interaction.mode === "move") return elementFromRootPoints(element, points.map((point) => ({ x: point.x + dx, y: point.y + dy })));
        const initialBounds = interaction.bounds!; const handle = interaction.handle ?? "se";
        const containsForm = interaction.base.some((item) => idSet.has(item.id) && formTools.includes(item.kind));
        const minWidth = containsForm ? 120 : 20;
        const minHeight = containsForm ? 38 : 20;
        const nextBounds = { ...initialBounds };
        if (handle.includes("w")) nextBounds.left = Math.max(0, Math.min(initialBounds.left + requestedDx, initialBounds.right - minWidth));
        if (handle.includes("e")) nextBounds.right = Math.min(root.width, Math.max(initialBounds.right + requestedDx, initialBounds.left + minWidth));
        if (handle.includes("n")) nextBounds.top = Math.max(0, Math.min(initialBounds.top + requestedDy, initialBounds.bottom - minHeight));
        if (handle.includes("s")) nextBounds.bottom = Math.min(root.height, Math.max(initialBounds.bottom + requestedDy, initialBounds.top + minHeight));
        const initialWidth = initialBounds.right - initialBounds.left;
        const initialHeight = initialBounds.bottom - initialBounds.top;
        return elementFromRootPoints(element, points.map((point, pointIndex) => ({
          x: nextBounds.left + (initialWidth > 0
            ? (point.x - initialBounds.left) / initialWidth
            : points.length > 1 ? pointIndex / (points.length - 1) : 0.5) * (nextBounds.right - nextBounds.left),
          y: nextBounds.top + (initialHeight > 0
            ? (point.y - initialBounds.top) / initialHeight
            : points.length > 1 ? pointIndex / (points.length - 1) : 0.5) * (nextBounds.bottom - nextBounds.top),
        })));
      });
  ...
  const pointerUp = () => {
    if (tool === "selection") {
      ...
      if (interaction?.last) setDraft((current) => {
        ...
        const elements = interaction.last!.map((element) => idSet.has(element.id) ? reanchorElement(element) : element);
        const history = [...current.history.slice(0, current.historyCursor + 1), elements].slice(-101);
        const next = { ...current, elements, history, historyCursor: history.length - 1, updatedAt: new Date().toISOString() };
  ...
  const pointerCancel = () => {
    ...
    interactionRef.current = undefined;
    if (interaction && interaction.mode !== "box") setDraft((current) => ({ ...current, elements: interaction.base }));
    setSelectionBox(undefined);
    setDrawing(undefined);
    setEraserPointer(undefined);
  };
  ...
  const viewportControls = pageLayerState === "exiting" ? null : <>
    ...
    {open && <div className="review-viewport-dock review-annotation-toolbar" role="toolbar" aria-label="批注工具栏">
      ...
      <button type="button" aria-label="选择" aria-pressed={tool === "selection"} className={tool === "selection" ? "active" : ""} onClick={() => setTool("selection")} title="选择"><MousePointer2 size={17} /></button>
  ...
  return <div className={`review-annotation-root ${open && tool === "selection" ? "selecting" : ""}`} ref={rootRef}>
    ...
    <svg className={`review-annotation-svg ${open && tool === "selection" ? "selecting" : ""} ${open && tool !== "browse" && tool !== "selection" ? "drawing" : ""}`} onPointerDown={pointerDown} onPointerMove={(event) => { if (tool === "eraser") setEraserPointer({ x: event.clientX, y: event.clientY }); pointerMove(event); }} onPointerUp={pointerUp} onPointerCancel={pointerCancel}>
      {geometries.map(({ element, left, top, width, height }) => {
        ...
        const common = { "data-annotation-element": element.id, stroke: element.color, strokeWidth: element.width, opacity: element.opacity, fill: "none" };
        if (formTools.includes(element.kind)) return <rect key={element.id} data-annotation-element={element.id} className="review-annotation-form-hit" x={Math.min(x1,x2)} y={Math.min(y1,y2)} width={Math.max(120, Math.abs(x2-x1))} height={Math.max(38, Math.abs(y2-y1))} />;
    ...
    {geometries.filter(({ element }) => ["text", "input", "select"].includes(element.kind)).map(({ element, left, top, width, height }) => {
      const first = element.points[0]; const last = element.points.at(-1) ?? first;
      const style = { left: left + Math.min(first.x, last.x) * width, top: top + Math.min(first.y, last.y) * height, width: Math.max(120, Math.abs(last.x-first.x) * width), height: Math.max(38, Math.abs(last.y-first.y) * height), color: element.color };
