...
import type { KnowledgeImportProgress, KnowledgeLibrary } from "./domain";
...
export default function KnowledgeSyncSettings({ uid, disabled }: { uid: string; disabled: boolean }) {
  ...
  const [pendingCopies, setPendingCopies] = useState<KnowledgeImportProgress[]>([]);
  ...
  useEffect(() => {
    const subscription = liveQuery(async () => {
      const available = await db.knowledgeLibraries.where("ownerScope").anyOf(["account:" + uid, "deviceGuest"]).toArray();
      const sessions = (await Promise.all(available.filter(library => library.ownerScope === "account:" + uid).map(library => db.knowledgeImportSessions.where("libraryId").equals(library.id).toArray()))).flat();
      return { available, sessions: sessions.filter(session => session.status === "active") };
    }).subscribe({
      next: ({ available, sessions }) => { setLibraries(available); setPendingCopies(sessions); },
  ...
  const copy = async () => {
    ...
    try {
      ...
      const sessions = await knowledgeRepository.listImportSessions(target.context);
      const active = sessions.find(session => session.sourceLibraryId === sourceId && session.status === "active");
  ...
  const continueCopy = async (session: KnowledgeImportProgress) => {
    if (busyRef.current || disabled || !window.confirm("继续已确认的复制？将使用开始复制时保存的内容，不要求来源库仍存在。")) return;
    busyRef.current = true; setBusy(true); setMessage("");
    try {
      if (currentKnowledgeOwner() !== "account:" + uid) { setMessage("账号已变化，请重新打开云同步设置。"); return; }
      const target = await knowledgeRepository.open(session.libraryId);
      await resumeKnowledgeImport(knowledgeRepository, target.context, session.id);
      setMessage("未完成的复制已继续完成，来源内容不受影响。");
    } catch (error) { setMessage(knowledgeUiError(error, "继续复制知识库").message); }
    finally { busyRef.current = false; setBusy(false); }
  };
  return <div className="knowledge-library knowledge-cloud-settings">
    ...
    {pendingCopies.map(session => <div key={session.libraryId + ":" + session.id}><p>未完成复制 · {libraries.find(library => library.id === session.libraryId)?.title} · {session.next}/{session.total}</p><button className="secondary-button" disabled={busy || disabled} onClick={() => void continueCopy(session)}>继续未完成复制</button></div>)}
