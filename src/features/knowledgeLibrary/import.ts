...
export const buildKnowledgeImport = (sourceState: KnowledgeState, scope: string, sessionId: string): KnowledgeCommand[] => {
  ...
  const ordered: KnowledgeEntity[] = Object.values(sourceState.entities).sort((left, right) => left.id < right.id ? -1 : left.id > right.id ? 1 : 0).filter(entity => entity.kind === "workspace");
  const waiting = Object.values(sourceState.entities).sort((left, right) => left.id < right.id ? -1 : left.id > right.id ? 1 : 0).filter(entity => entity.kind === "node");
  ...
  ordered.push(...Object.values(sourceState.entities).sort((left, right) => left.id < right.id ? -1 : left.id > right.id ? 1 : 0).filter(entity => entity.kind === "reference"));
  ...
  for (const entity of ordered) {
    for (const candidate of Object.values(sourceState.candidates).sort((left, right) => left.id < right.id ? -1 : left.id > right.id ? 1 : 0).filter(item => item.consumedBy === null && sourceState.revisions[item.revisionId].entityId === entity.id)) {
...
export const prepareKnowledgeImport = async (repository: KnowledgeRepository, sourceLibraryId: string, target: KnowledgeContext, sessionId: string): Promise<void> => {
  ...
  await database.transaction("rw", knowledgeTables(database), async () => {
    ...
    const commands = buildKnowledgeImport(sourceState, scope, sessionId);
...
export const resumeKnowledgeImport = async (repository: KnowledgeRepository, context: KnowledgeContext, sessionId: string, onProgress?: (completed: number, total: number) => void | Promise<void>, assertActive?: () => Promise<void>): Promise<void> => {
  while (true) {
    ...
    await onProgress?.(session.next, session.total);
    ...
    await repository.execute(context, step.command, undefined, sessionId, assertActive);
  }
};
