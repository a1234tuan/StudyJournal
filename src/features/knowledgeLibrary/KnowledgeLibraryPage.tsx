...
export const KnowledgeLibraryPage = ({ records, assets = [], navigation, onNavigation, onBack, onOpenRecord }: Props) => {
  ...
  const move = async (id: string, parentNodeId: string) => {
    ...
    await knowledgeRepository.execute(context, editKnowledgeEntity(scope, state, entity, "position", { parentNodeId, orderKey: orderBetween(last ? (valueOf(state, last, "position") as KnowledgePosition).orderKey : null, null, crypto.randomUUID()) }));
  ...
  return <main className={"page knowledge-library knowledge-shell" + (workspace ? " knowledge-editor" : "")} aria-busy={busy || loading}>
    ...
    {loading ? <p role="status">正在读取…</p> : !workspace ? <section className="knowledge-home"><div className="knowledge-home-actions">{library && <button className="knowledge-library-switch" onClick={() => openPanel("manage")}><span>当前库：{library.title}</span><ChevronRight size={16} /></button>}{!!topics.length && <button className="primary-button" onClick={beginTopic}><Plus size={17} />新建专题</button>}</div>{!topics.length ? <div className="knowledge-empty"><BookOpen size={34} strokeWidth={1.4} /><h2>从一个专题开始</h2><button className="primary-button" onClick={beginTopic}>创建第一个专题</button></div> : <div className="knowledge-topics">{topics.map(topic => <button className="knowledge-topic" key={topic.id} onClick={() => onNavigation({ libraryId: library?.id, workspaceId: topic.id, selectedNodeId: undefined, scrollTop: 0, collapsed: [] }, true)}><BookOpen size={20} /><strong>{knowledgeLabel(state, topic)}</strong><ChevronRight size={17} /></button>)}</div>}</section> : <>
      <div className="knowledge-workspace">
        {navigation.view === "outline" ? <KnowledgeOutline state={state} workspaceId={workspace.id} navigation={navigation} editor={titleEditor} onSelect={id => { if (!editing) onNavigation({ selectedNodeId: id }); }} onEdit={id => { if (!editing) void run(() => beginEdit(state.entities[id], "title")); }} onCreate={beginCreate} onOutdent={id => { const position = valueOf(state, state.entities[id], "position") as KnowledgePosition; if (position.parentNodeId === ROOT_NODE) return; const parent = state.entities[position.parentNodeId]; void run(() => move(id, (valueOf(state, parent, "position") as KnowledgePosition).parentNodeId)); }} onToggle={id => onNavigation({ collapsed: navigation.collapsed.includes(id) ? navigation.collapsed.filter(item => item !== id) : [...navigation.collapsed, id] })} onScrollPosition={scrollTop => onNavigation({ scrollTop })} /> : <><KnowledgeMap state={state} workspaceId={workspace.id} navigation={navigation} organizing={!editing} editor={titleEditor} onEdit={id => void run(() => beginEdit(state.entities[id], "title"))} onNavigation={onNavigation} onSelect={id => { if (!editing) onNavigation({ selectedNodeId: id }); }} onToggle={id => onNavigation({ collapsed: navigation.collapsed.includes(id) ? navigation.collapsed.filter(item => item !== id) : [...navigation.collapsed, id] })} onMove={(id, parent) => void run(() => move(id, parent))} />{titleEditor?.isNew && <div className="knowledge-map-new"><KnowledgeTitleInput editor={titleEditor} label="新建节点" /></div>}</>}
        {detailOpen && selected && !modal && <aside className="knowledge-detail" aria-label="节点详情"><div className="knowledge-panel-heading"><h2>{knowledgeLabel(state, selected)}</h2><button className="knowledge-icon" aria-label="关闭节点详情" onClick={() => setDetailOpen(false)}><X size={19} /></button></div><button className="knowledge-note-action" onClick={() => void run(() => beginEdit(selected, "note"))}>{String(valueOf(state, selected, "note") || "添加节点说明")}</button>{references.map(reference => { const record = recordMap.get(reference.recordId); return <article className="knowledge-reference" key={reference.id}><button className="knowledge-reference-link" disabled={!record || Boolean(record.deletedAt)} onClick={() => record && onOpenRecord(record)}>{record?.title || "原日志暂不可用"}</button>{!record && <small>等待原日志同步</small>}{record?.deletedAt && <small>原日志在回收站</small>}{valueOf(state, reference, "remark") && <p className="knowledge-note">{String(valueOf(state, reference, "remark"))}</p>}<details><summary aria-label={"引用操作：" + (record?.title || "暂不可用")}>更多</summary><button onClick={() => void run(() => beginEdit(reference, "remark"))}>编辑备注</button><button onClick={() => void changeLifecycle(reference, true)}>移除引用</button></details></article>; })}<button className="secondary-button" onClick={openPicker}>添加日志</button></aside>}
      </div>
      <footer className="knowledge-contextbar">{selected ? <><span className="knowledge-selection-title">{knowledgeLabel(state, selected)}</span><button disabled={Boolean(editing)} onClick={() => beginCreate(selected.id)}><Plus size={16} />子节点</button><button disabled={Boolean(editing)} onClick={openPicker}>添加日志</button><button disabled={Boolean(editing)} onClick={() => setDetailOpen(!detailOpen)} aria-pressed={detailOpen}>详情{references.length ? " · " + references.length : ""}</button><button className="knowledge-icon" disabled={Boolean(editing)} aria-label="更多节点操作" onClick={() => openPanel("node-menu")}><MoreHorizontal size={20} /></button></> : <button disabled={Boolean(editing)} onClick={() => beginCreate()}><Plus size={17} />添加节点</button>}<span className="knowledge-save-state" role="status">{busy ? "保存中…" : library?.cloudLibraryId ? "账号同步库" : "本机保存"}</span></footer>
    </>}
