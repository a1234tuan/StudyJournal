...
import { emptyKnowledgeState, ROOT_NODE, type KnowledgeCommand, type KnowledgeContext, type KnowledgeEntity, type KnowledgeLibrary, type KnowledgePosition, type KnowledgeState, type KnowledgeSyncFailure, type KnowledgeUnit } from "./domain";
...
export const KnowledgeLibraryPage = ({ records, assets = [], navigation, onNavigation, onBack, onOpenRecord }: Props) => {
  ...
  return <main className={"page knowledge-library knowledge-shell" + (workspace ? " knowledge-editor" : "")} aria-busy={busy || loading}>
    {recoveryProtected && <section className="knowledge-feedback" role="status"><p>此恢复副本正在保全待确认内容，只能查看。</p><button disabled={busy} onClick={() => void run(async () => { if (!library) return; const copy = await knowledgeRepository.createLibrary("可编辑保留副本"); const target = await knowledgeRepository.open(copy.id); const sessionId = crypto.randomUUID(); await prepareKnowledgeImport(knowledgeRepository, library.id, target.context, sessionId); await resumeKnowledgeImport(knowledgeRepository, target.context, sessionId); afterClose(() => onNavigation({ libraryId: copy.id, workspaceId: undefined, selectedNodeId: undefined, scrollTop: 0 }, true)); })}>另存可编辑副本</button></section>}
    ...
    {!modal && feedback}
    {syncFailure && <section className="knowledge-feedback" role="alert"><p>云提交校验失败，知识库同步已停止。本机内容和待同步操作仍保留。</p><p>最后可信序号：{syncFailure.cursor} · 失败类别：{syncFailure.code} · 连续失败：{syncFailure.attempts} 次</p><button disabled={busy} onClick={() => void run(async () => { const sync = knowledgeSynchronizer(); if (context && sync) await sync.synchronize(context); }, "知识库重新校验完成", "重试知识同步")}>重新校验</button><button disabled={busy} onClick={() => void run(async () => { if (!context) return; const id = await knowledgeRepository.preserveSyncFailure(context); afterClose(() => onNavigation({ libraryId: id, workspaceId: undefined, selectedNodeId: undefined, scrollTop: 0 }, true)); })}>保全并打开本机副本</button><p>可通过“备份与恢复”导出当前内容。持续失败需管理员从可信备份修复云端历史，应用不会跳过坏提交或新建替代云库。</p></section>}
