...
- The voice-recall production workspace uses a real `ASR -> LLM -> TTS` pipeline with device-local credentials, real transcript/reply streaming, manual input modes, and automatic half-duplex mode. Automatic half-duplex has been validated on an Android physical device across multiple consecutive rounds, including automatic ASR, LLM submission, TTS playback, and re-listening. AI playback remains microphone-muted and barge-in is not implemented. The desktop host owns the ASR WebSocket (`study-journal:voice-asr-*`) and Android uses the native capture/ASR path. SiliconFlow real-time ASR, Doubao TTS, and PCM/Opus gapless playback remain unimplemented. Manual push-to-talk/tap-to-record, keyboard/caption input, pause/continue/end actions, and local-log retention actions remain physical-device gates. `?preview=voice-recall` remains the isolated Mock call-control prototype. See `docs/voice-recall-freeze-2026-09-11.md` and `docs/realtime-voice-recall-implementation.md`.
