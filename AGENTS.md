...
- The 2026-09-09 voice-recall follow-up replaces the simulated call with a real `ASR -> LLM -> TTS` pipeline. The production workspace resolves device-local credentials, refuses to start when a required stage is unavailable, streams real transcripts/replies, and supports manual input plus a candidate automatic half-duplex mode; AI playback remains microphone-muted and barge-in is not implemented. The desktop host owns the ASR WebSocket (`study-journal:voice-asr-*`) and Android uses `NativeVoiceAsrPlugin` (OkHttp). Aliyun Paraformer ASR, DeepSeek LLM and Fish Audio TTS were verified against the live services on 2026-09-09; Doubao speech is retained as a candidate but its app is not activated. `?preview=voice-recall` remains the isolated Mock call-control prototype. Physical-device acceptance of the real chain is still a release gate. See `STUDYJOURNAL_FIX_PLAN.md` and `docs/realtime-voice-recall-implementation.md`.
...
- The deterministic `voice-mock-cn@1` template is verified for automation. `voice-default-cn@2` combines Aliyun Paraformer ASR, the user's configured LLM and Fish Audio TTS; those three legs were verified against the live services on 2026-09-09, but the template stays `candidate` until physical-device acceptance. Doubao speech profiles remain candidates because the tested app id is not activated for streaming ASR/TTS; never claim model, voice, quota, cost, browser-direct support, or streaming behavior from configuration alone.
