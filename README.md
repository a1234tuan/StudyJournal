...
> 本文按 APP 内“更多 → 使用教程”整理，并补充当前版本的今日计划和语音服务说明。插图为应用内的流程说明，并非界面截图。更新日期：2026-09-21。
...
本次仓库发布标识为 **v0.2.5**；已有安装包内部版本沿用 Android **0.2.3（versionCode 14）**、Windows **0.1.6**。发布标识和平台内部版本不是同一套编号，请以发布说明、构建记录和 SHA-256 判断产物，而不是只看应用显示的版本号。
...
- **图片文字**：在“更多 → OCR 设置”配置服务后进行识别，已识别文本可参与检索和 AI 上下文。
- **集中回听**：从侧栏“录音”或“更多 → 录音库”查找日志录音和知识播客；原始内容仍关联对应日志。
...
> 2026-09-25 开发分支：知识库已接入一键同步。电脑保存后同步，手机使用同一账号同步即可拉取；正常流程不再选来源或手动复制。旧来源只读保留，恢复副本不自动上传。此说明不代表已发布的 v0.2.5 安装包包含该改动；部署及真机验收状态见 [实施记录](docs/knowledge-library-one-click-sync-implementation-2026-09-25.md)。
...
- **录音在哪里？** 打开侧栏“录音”或“更多 → 录音库”，也可以回到对应日志。
